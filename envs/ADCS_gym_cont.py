...
class AttitudeControlEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if not done:
            ...
            if qs > self.qs_prev:
                reward = (.5+(1-(self.max_episode_steps/5-self.nsteps/5)/(self.max_episode_steps/5+1)))*np.exp(-arg) - (.5+(self.max_episode_steps/5-self.nsteps/5)/(self.max_episode_steps/5+1))*0.5*(np.linalg.norm(action))
                #print("1",reward)
            else:
                reward = (.5+(self.max_episode_steps/5-self.nsteps/5)/(self.max_episode_steps/5))*np.exp(-arg) - (1.5-(self.max_episode_steps/5-self.nsteps/5)/(self.max_episode_steps/5))*0.5*(np.linalg.norm(action)) - 1.
                #print("0",reward)
